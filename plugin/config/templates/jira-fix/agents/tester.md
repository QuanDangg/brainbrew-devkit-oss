...
Route based on test result AND the `scope` value from bug-classifier
(propagated through Previous Agent Output):

| Result | Scope | Chain effect |
|--------|-------|-------------|
| **FAIL** or build error | any | → debugger |
| **PASS** | frontend or fullstack | → ui-verifier |
| **PASS** | backend | → code-reviewer |

If scope is missing from previous context, treat it as `fullstack`
(safer — ui-verifier will still run).
...
- Do NOT skip — if no tests exist in the project, report
  `no test suite configured` and PASS; downstream ui-verifier /
  code-reviewer are the real gate in that case
